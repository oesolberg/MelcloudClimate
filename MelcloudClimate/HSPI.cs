...
namespace HSPI_MelcloudClimate
{
    ...
    public class HSPI : HspiBase2
    {
        ...
        private void CreateMelcloudDevice(dynamic device)
        {
            ...
            ClimateDevices.Add((string)device.DeviceID, new Dictionary<String, Device>());

            JsonCommand.Add((string)device.DeviceID, new JObject()); //Create a new object

            JsonCommand[device.DeviceID.ToString()].DeviceID = device.DeviceID;
            JsonCommand[device.DeviceID.ToString()].BuildingId = device.BuildingID;
            JsonCommand[device.DeviceID.ToString()].HasPendingCommand = false;
            ...
            ClimateDevices[(string)device.DeviceID].Add("RootDevice", RootDevice);

            //Set the device to the modus picked up

            Console.WriteLine("Parent id " + device.DeviceID);
            ...
            ClimateDevices[(string)device.DeviceID].Add("CurrentTemperatureDevice", CurrentTemperatureDevice);
            ...
            JsonCommand[device.DeviceID.ToString()].SetTemperature = device.Device.SetTemperature;
            ClimateDevices[(string)device.DeviceID].Add("SetpointTemperatureDevice", SetpointTemperatureDevice);
            ...
            //Get current setpoint
            JsonCommand[device.DeviceID.ToString()].OperationMode = device.Device.OperationMode;

            ClimateDevices[(string)device.DeviceID].Add("OperationalModeDevice", OperationalModeDevice);
           ...
           // JsonCommand[device.DeviceID.ToString()].FanSpeed = device.Device.FanSpeed;
            ...
            ClimateDevices[(string)device.DeviceID].Add("FanSpeedDevice", FanSpeedDevice);
        ...
        private void GetDevices(int retry = 0)
        {
            ...
            try
            {
                ...
                if ((int)response.StatusCode == 200)
                {
                    ...
                    Log.Debug("Process all devies pr floor");
                    ...
                    Log.Debug("Process all devies pr area");
                    ...
                    Log.Debug("Process all devies pr clients");
                    ...
                    Log.Debug("Process all devies pr devices");
